...
public class ProjectService : IProjectService
{
    ...
    public async Task<ProjectModel> CreateAsync(ProjectModel projectModel)
    {
        ...
        if (await _userRepository.GetByIdAsync(project.CreatorId) == null)
        {
            throw new InvalidOperationException($"User with id '{project.CreatorId}' does not exist");
        }
        
        if (await _categoryRepository.GetByIdAsync(project.CategoryId) == null)
        {
            throw new InvalidOperationException($"Category with id '{project.CategoryId}' does not exist");
        }
    ...
    public async Task<ProjectModel> UpdateAsync(Guid id, ProjectModel projectModel)
    {
        ...
        if (await _projectRepository.GetByIdAsync(project.CategoryId) == null)
        {
            throw new InvalidOperationException($"Category with id '{project.CategoryId}' does not exist");
        }
