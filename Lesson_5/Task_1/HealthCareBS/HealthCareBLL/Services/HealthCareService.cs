...
public class HealthCareService : IHealthCareService
{
    ...
    public async Task<HealthCareDto> GetHealthCareRecordById(Guid id)
    {
        ...
        {
            throw new KeyNotFoundException($"Health care with id '{id}' not found");
        }
    ...
    {
        var healthCareRecords = await _healthCareRepository.GetByIdAsync(id);
        
        if (healthCareRecords == null)
        {
            throw new KeyNotFoundException($"Health Care with id '{id}' was not found");
        }
        
        var deletedHealthCareRecords = await _healthCareRepository.DeleteAsync(healthCareRecords);
        
        return _mapper.Map<HealthCareDto>(deletedHealthCareRecords);
    }
